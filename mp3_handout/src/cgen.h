...
public:
  ...
  void close_scope() { var_table.exitscope(); type_table.enterscope();}
  ...
  llvm::Type* getType(Symbol name) {
      return type_table.find_in_scopes(name);
  }

  void add_type(Symbol name, llvm::Type* type) {
    type_table.insert(name, type);
  }

  llvm::Value* getVal(Symbol name) {
